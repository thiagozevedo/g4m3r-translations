...
module.exports = class extends Language {
	...
	constructor(...args) {
		...
		this.language = {
			WARNINGS: 'Avisos',
			AFK_HELP_DESCRIPTION: 'Ativa / desativa ou define uma mensagem de AFK para voce mesmo.',
			AFK_HELP_EXTENDED: prefix =>
				[
					`**${prefix}afk** ativa ou desativa sua mensagem de AFK`,
					`**${prefix}afk sua mensagem aqui** define a nova mensagem de AFK que será enviada`,
					'',
					'Avançado: Você tambem pode definir sua mensagem com um embed customizado!'
				].join('\n'),
			AFK_MESSAGE_UPDATED: 'Você atualizou sua mensagem de AFK com sucesso.',
			AFK_STATUS_TOGGLED: status => `Você **${status ? 'ATIVOU' : 'DESATIVOU'}** sua mensagem de AFK com sucesso.`,
			ANNOUNCE_HELP_DESCRIPTION: 'Envia um anúncio marcando um cargo sem precisar torná-lo mencionável para todos do servidor.',
			ANNOUNCE_CONTENT_REQUEST: 'Por favor, digite a mensagem que você gostaria de enviar. **Dica**: Você tambem pode usar um embed, colando um código de embed.',
			ANNOUNCE_RESPONSE_AUTHOR_TEXT: memberName => `Anúncio por: ${memberName}`,
			ANNOUNCE_RESPONSE_FAILED: 'Não pude enviar uma mensagem para o canal que você providenciou. Por favor, verifique as permissões para ter certeza de que eu tenho as permissões necessárias.',
			ANNOUNCE_RESPONSE_SUCCESS: 'O anúncio foi enviado com sucesso.',
			COMMANDS_SERVER: {
				title: guildName => `Status do servidor « ${guildName} »`,
				statsName: 'Este servidor',
				statsValue: (createdAt, owner, channels, roles, members, bots) =>
					[`Criado [**${createdAt}**]`, `👑 [**${owner}**]`, `Canais [**${channels}**]`, `Cargos [**${roles}**]`, `👥 [**${members}**]`, `🤖 [**${bots}**]`].join('\n'),
				settingsName: 'Configurações básicas G4M3R',
				settingsValue: settings =>
						[
								`Idioma: ${settings.language}`,
								`Cargos de Moderador: ${settings.settings.moderation.modRoles.map(roleID => `<@&${roleID}>`).join(' ')}`,
								`Administradores: ${settings.settings.admins.map(userID => `<@${userID}>`).join(' ')}`,
								`Permitir mensagens de AFK: ${settings.settings.allowAfkResponses}`,
								`Recurso "Feedback": **Ideia:** ${settings.settings.feedback.idea.status} | **Problemas**: ${settings.settings.feedback.bug.status}`
						].join('\n'),
				moderationName: 'Configurações de Moderação G4M3R',
				moderationValue: settings =>
						[
								`Cargo Auto Atribuído: ${settings.settings.autoAssignRoles.mainRole ? `<@&${settings.settings.autoAssignRoles.mainRole}>` : ''}`,
								`Cargos Auto-atribuíveis: ${settings.saRoles.length ? settings.saRoles.map(roleID => `<@&${roleID}>`).join(' ') : 'Nenhum'}`,
								`Cargos Auto-atribuíveis por Mods: ${settings.assignRoles.length ? settings.assignRoles.map(roleID => `<@&${roleID}>`).join(' ') : 'Nenhum'}`,
								`Cargo de Verificação: ${settings.settings.verify.role ? `<@&${settings.settings.verify.role}>` : 'Nenhum'}`,
								`Categoria de Mail: ${settings.settings.mail.mailCategory ? settings.settings.mail.mailCategory : 'Nenhum'}`,
								`Categoria de Verificação: ${settings.settings.verify.category}`,
								`Canais de Verificação abertos: ${settings.welcomeChannels.length}`
								// `Verification Main Channel: ${settings.channels.verifyMainChannelID}`,
						].join('\n')

			},
			BAD_EMBED: 'Isso não é um **objeto embed válido**. Tente usar o construtor de embeds em <https://embedbuilder.nadekobot.me/>!',
			CS: 'CS',
			DAYS: 'Dias',
			DEFEAT: 'Derrota',
			DURATION: 'Duração',
			EVENTS_ADD_ROLE_QUESTION: 'Por favor, providencie os @cargos, ID dos cargos, ou nome dos cargos que você gostaria de usar. Todos os membros com este cargo serão adicionados ao evento.',
			EVENT_PLATFORM_QUESTION: 'Qual plataforma você gostaria de usar para este evento?',
			FEEDBACK_DEFAULT: action => `**${klasaUtil.toTitleCase(action)}** Feedback`,
			FEEDBACK_EDIT_NOT_FOUND: id => `Não pude encontrar a mensagem de Feedback com o ID: ${id}`,
			FEEDBACK_EDIT_NOT_AUTHOR: 'Você não pode editar este Feedback porque não é o autor.',
			FEEDBACK_EDIT_SUCCESS: 'Você editou seu Feedback com sucesso.',
			FEEDBACK_ORIGINAL_NOT_FOUND: 'Não encontrei esta mensagem de Feedback neste servidor; logo, não foi possível editá-la.',
			FEEDBACK_FOOTER: (prefix, action, id) => `Para enviar feedback: ${prefix}fb ${action} | Para editar feedback: ${prefix}fb edit ${id}`,
			FEEDBACK_FROM: (type, authorName) => `${klasaUtil.toTitleCase(type)} De ${authorName}`,
			FEEDBACK_NOT_ACTIVATED:
				'Este servidor não tem esta opção de feedback habilitada. Por favor, entre em contato com um moderador ou administrador para habilitar este recurso.',
			FEEDBACK_NOT_VALID_ACTION: (action, validActions) =>
				`**${action}** não é uma ação válida. Tente novamente com uma das seguintes opções:\n\n${validActions.join('\n')}`,
			FEEDBACK_NO_CONTENT: 'Você não providenciou o feedback a ser enviado. Por favor, tente novamente e inclua a mensagem que você gostaria de enviar.',
			FEEDBACK_SENT: 'Seu feedback foi enviado, obrigado!',
			GAMES_PLAYED: 'Partidas jogadas',
			HOURS: 'Horas',
			IDEA: 'Ideia',
			KDA: 'KDA',
			MAINTAINANCE: 'O bot está em manutenção, se preparando pra uma atuaização bem legal. Por favor, aguarde uns instantes!',
			MENU_CANCEL_OPTIONS: ['s', 'sair'],
			MINUTES: 'Minutos',
			NOT_A_MOD: 'Você não é um moderador neste servidor. Por favor, entre em contato com um dos moderadores ou administradores.',
			REACTIONROLES_ADDING_FAILED: name => `Não foi possível salvar a mensagem com cargos de reações "${name}" no banco de dados.`,
			REACTIONROLES_ADDING_NEED_EMOJI_NAME:
				"Qual o nome do emoji que você gostaria de usar? \n**Dica: O emoji deve estar salvo no banco de dados.**\nPara salvar emojis, use o comando **emoji**.",
			REACTIONROLES_ADDING_NEED_ROLES: 'Por favor, providencie os cargos que você quer que o usuário receba ao clicar neste emoji.',
			REACTIONROLES_ADDING_NEED_ACTION: 'Você gostaria de adicionar mais emojis?\n\n**Sim ou Não?**',
			REACTIONROLES_ADDING_NEED_CONTENT: 'Por favor, providencie o código embed com a mensagem que você quer usar para estes emojis e cargos.',
			REACTIONROLES_ADDING_SUCCESS: name => `Você registrou a mensagem com cargos de reações "${name}" com sucesso.`,
			REACTIONROLES_INVALID_EMOJI_NAME:
				'Não pude encontrar um emoji com este nome no meu banco de dados. Por favor, adicione todos os emojis que você gostaria de usar antes de criar esta mensagem com cargos e reações.',
			SECONDS: 'Segundos',
			SETTINGS_EVENTS_MINUTES_FROM_NOW_QUESTION: current =>
				`Quantos minutos após a criação você quer que seus eventos comecem? Atualmente a quantidade padrão de minutos é ${current}.`,
			SETTINGS_EVENTS_MINUTES_FROM_NOW_RESPONSE: minutes =>
				`Você definiu que os eventos começarão ${minutes} minutos contando a partir do momento em que foram criados.`,
			SETTINGS_FEEDBACK_IDEA_ALLOWED_ACTIONS: actions =>
				`Por favor, diga a palavra alternativa que você gostaria de usar para **ideia**.\n\n **Palavras atuais:**\n\n${actions.length ? actions.join(', ') : 'Nenhuma palavra.'}`,
			...
			SETTINGS_FEEDBACK_ALLOWED_ACTION_SUCCESS: (type, added, removed, skipped) =>
				[
					'',
					`${added.length ? `**Palavras adicionadas a ${type === 'idea' ? 'Ideia' : 'Bug'}**\n\n${added.join(', ')}` : ''}`,
					`${removed.length ? `**Removed Words To ${type === 'idea' ? 'Ideia' : 'Bug'}**\n\n${removed.join(', ')}` : ''}`,
					`${skipped.length ? `**Skipped Words To ${type === 'idea' ? 'Ideia' : 'Bug'}**\n\n${skipped.join(', ')}` : ''}`
				].join('\n'),
			SETTINGS_FEEDBACK_QUESTIONS: (type, questions) =>
				`Providencie uma pergunta para ser adicionada ou removida da lista de perguntas de ${type}.\n\n**Perguntas atuais:**\n\n${questions.length ? questions.join('\n') : 'Nenhuma pergunta'}`,
			SETTINGS_FEEDBACK_QUESTION_SUCCESS: (type, added) => `Você **${added ? 'adicionou' : 'removeu'}** a pergunta da lista de perguntas de ${type} com sucesso.`,
			SETTINGS_FEEDBACK_MESSAGE_QUESTION: (type, item, message) =>
				`Qual mensagem você quer enviar ao usuário quando ele clicar no ${
			...
			SETTINGS_FEEDBACK_MESSAGE_SUCCESS: (type, item) =>
				`A mensagem de feedback de **${type}** ao clicar na reação ${
					item === 'solvedMessage' ? this.client.functions.constants.EMOJIS.GREEN_CHECK : this.client.functions.constants.EMOJIS.RED_X
				} foi modificada com sucesso.`,
			SETTINGS_FEEDBACK_XP_QUESTION: (emoji, points) => `Quando de XP deve ser modificado quando ${emoji} for clicado?\n\n**Mensagem atual:**\n\n${points}`,
			SETTINGS_FEEDBACK_XP_SUCCESS: (emoji, points) => `${points} serão editados quando ${emoji} for clicado.`,
			SETTINGS_LEVELS_MENU_LIST_DATA: xp => [
				'Criar Nívell',
				'Deletar Nível',
				'Editar Nome do Nível',
				'Editar XP necessário p/ nível',
				'Editar Recompensas de Nível',
				'Editar XP por Comando',
				`XP Por Mensagem [${xp.pointsPerMessage}]`,
				`Server: Notificação **No canal** [**${xp.notification.server.channel ? 'On' : 'Off'}**]`,
				`Server: Notificação por **Mensagem Privada** [**${xp.notification.server.dm ? 'On' : 'Off'}**]`,
				`Global: Notificação **No canal** [**${xp.notification.global.channel ? 'On' : 'Off'}**]`
			],
			SETTINGS_NOTIFICATION_UPDATE_SERVER_RESPONSE: (activated, isChannel) =>
				`Notificações ${isChannel ? 'no canal' : 'por Mensagem Privada'} para **subida de nível de XP no servidor** foram **${activated ? 'ATIVADAS' : 'DESATIVADAS'}** neste servidor.`,
			SETTINGS_NOTIFICATION_UPDATE_GLOBAL_RESPONSE: activated =>
				`Notificações no canal para **subida de nível de XP global** foram **${activated ? 'ATIVADAS' : 'DESATIVADAS'}** neste servidor.`,
			SETTINGS_LEVEL_CREATE_NAME_QUESTION: 'Qual nome você gostaria de dar para este nível?',
			SETTINGS_LEVEL_CREATE_NUMBER_QUESTION: 'Quanto XP um usuário precisa para chegar neste nível?',
			SETTINGS_LEVEL_CREATE_ROLE_QUESTION:
				'Por favor, mencione um @cargo ou diga um ID de cargo ou o nome do cargo que voce gostaria de usar como recompensa por atingir este nível. Se você não deseja dar um cargo, diga [s]alvar.',
			SETTINGS_LEVEL_CREATE_SUCCESS: (name, xpNeeded, roles) =>
				`Você criou com sucesso o nível **${name}** com **${xpNeeded}** XP necessário para ser alcaçado${typeof roles === 'string' ? '' : ` e ganhar o(s) cargo(s) ${roles}`}`,
			SETTINGS_LEVEL_DELETE_SELECT_LEVEL_TITLE: 'Por favor, selecione o nível que você gostaria de deletar',
			SETTINGS_LEVEL_DELETE_SUCCESS: level => `Você deletou o nível ${level}`,
			SETTINGS_LEVEL_EDIT_NAME_QUESTION: name => `Qual você deseja que seja o nome nome deste nível? O nome atual é **${name}**.`,
			SETTNGS_LEVEL_EDIT_NAME_SUCCESS: name => `Você atualizou com sucesso o nome deste nível para **${name}**.`,
			SETTINGS_LEVEL_EDIT_NUMBER_QUESTION: xpNeeded => `Qual você deseja que seja a quantida de XP necessária para alcançar este nível? XP necessário atualmente: **${xpNeeded}**`,
			SETTNGS_LEVEL_EDIT_XP_NEEDED_SUCCESS: xpNeeded => `Você atualizou com sucesso a quantidade de XP necessária para: **${xpNeeded}**.`,
			SETTINGS_LEVEL_EDIT_ROLE_QUESTION: roles => `Qual cargo você gostaria de adicionar ou remover?\n\n${roles.length ? `**Recompensas atuais:** ${roles.join('\n')}` : ''}`,
			SETTNGS_LEVEL_EDIT_ROLE_SUCCESS: roles => `Você atualizou com sucesso os cargos que serão utilizados para este nível:\n\n**Cargos:**\n\n${roles}`,
			SETTINGS_LEVEL_PICK_LEVEL: 'Por favor, escolha o nível que você gostaria de editar',
			SETTINGS_LEVEL_PICK_COMMAND: 'Por favor, escolha o comando que você gostaria de editar',
			SETTINGS_LEVEL_INVALID_LEVEL: level => `O nível **${level}**, que você selecionou não existe. Tente novamente.`,
